	...
	async function rastrearEncomenda() {
		...
		for (codigo in req.query) {
			...
			if (encomenda[0].error === "moved_temporarily") {
				respostaTotal.push(
					`O sistema dos correios moveu as informações da encomenda ${encomenda[0].code}. Sinto muito.`
				);
				continue;
			}
			...
			let textoResposta = `O objeto de código ${
				encomenda[0].code
			} foi visto pela última vez em ${
				ultimoRastreio.locale
			} em ${ultimoRastreio.trackedAt.toLocaleString()}, com o status de ${
				ultimoRastreio.status
			}.`;
